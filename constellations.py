...
def make(Cosntellation):
	Clines_f = map(lambda s: s[:-1].split(), Clines )
	...
	df = dat.to_pandas().set_index('HIP')
	df = df.loc[np.unique(line_arr)]
	#print(df)

	RA_center = ( np.max(df['_RA_icrs']) + np.min(df['_RA_icrs']) )/2.  
	...
	loc = EarthLocation(lat=DE_center*u.deg, lon=0*u.deg)
	...
	for hip, star in df.iterrows():  #make stars
		r = 8*1.3**(-star['Vmag'])
		if r>15: r=15
		sphere = pymesh.generate_icosphere(r,(star['x'],star['y'],0),refinement_order=2)#.scale(1,1,0.5)
		sphere = pymesh.form_mesh(sphere.vertices*np.array((1,1,0.75)), sphere.faces);
		star_list.append(sphere)
	#	mesh = pymesh.boolean(mesh,sphere,operation='union')
